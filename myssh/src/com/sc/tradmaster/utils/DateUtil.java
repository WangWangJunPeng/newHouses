...
public class DateUtil {
    ...
    public static final String PATTERN_CLASSICAL_SIMPLE_YMD = "yyyy年MM月dd日";
	...
	public static List<String> getTwoDateEveryDay(String startTime, String endTime) {
		...
		Date startDate = parse(startTime, PATTERN_CLASSICAL_SIMPLE);
		Date endDate = parse(endTime, PATTERN_CLASSICAL_SIMPLE);
		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");

		Long i = startDate.getTime();
		list.add(df.format(startDate));
		list.add(df.format(rollDay(startDate, 1)));
		while (rollDay(new Date(i), 1).getTime() < endDate.getTime()) {

			i = rollDay(new Date(i), 1).getTime();
			list.add(df.format(rollDay(new Date(i), 1)));
    ...
    public static void main(String[] args) throws Exception{
    	...
    	//输出数据为2010-09-09
			...
			//System.out.println(format(new Date("2017/3/8")));
    	//System.out.println(addDateMinut("2013-08-29 08:16:36",8));
    	
    	/*SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");//设置日期格式
		Date start = df.parse("2013-12-04");
    	...
    	System.out.println(rollMonth(new Date(), -3));*/
    	String startTime = "2017-02-01";
    	String endTime = "2017-03-05";
    	Date startDate = DateUtil.parse(startTime, PATTERN_CLASSICAL_SIMPLE);
		Date endDate = DateUtil.parse(endTime, PATTERN_CLASSICAL_SIMPLE);
		String start1 = DateUtil.format(startDate);
		Date s = DateUtil.parse(start1, DateUtil.PATTERN_CLASSICAL);
		Date d = DateUtil.getIntegralEndTime(s);
		
		System.out.println(format(d));
    	/*for(long i = startDate.getTime(); i <= endDate.getTime(); DateUtil.rollDay(new Date(i), 7).getTime()){
			System.out.println(i);
			System.out.println(df.format(new Date(i)));
			
			
			
			i = rollDay(new Date(i), 7).getTime();
			
			System.out.println(df.format(new Date(i)));
		}*/  
    	/*long i = startDate.getTime();
    	System.out.println(df.format(new Date(i)));
    	while(rollDay(new Date(i), 7).getTime() < endDate.getTime()){
    		//System.out.println(i);
    		...
    		int vv = getOffsetDays(new Date(i), endDate);
    		if(vv <= 7){
    			for(int v = 1; v < vv; v++){
    				
    				System.out.println(" - ");
    			}
    			System.out.println(df.format(new Date(endDate.getTime())));
    		}
    		//System.out.println(rollDay(startDate, 7).getTime());
    	}*/
		
		/*long i = startDate.getTime();
        System.out.println(df.format(new Date(i)));
        while(DateUtil.rollMonth(new Date(i), 3).getTime() < endDate.getTime()){
        	//System.out.println(getOffsetDays(new Date(i), DateUtil.rollMonth(new Date(i), 3)));
        	for(int x = 0; x < getOffsetDays(new Date(i), DateUtil.rollMonth(new Date(i), 3))-1; x++){
         ...
         while(DateUtil.rollMonth(new Date(i), 1).getTime() < endDate.getTime()){
        	 ...
        	 for(int x = 0; x < DateUtil.getOffsetDays(new Date(i), DateUtil.rollMonth(new Date(i), 1))-1; x++){
		...
		/*long i = startDate.getTime();
    	System.out.println(df.format(new Date(startDate.getTime())));
