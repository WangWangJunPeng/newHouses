...
public class AppTagManagerServiceImpl implements AppTagManagerService{
	...
	public PCustomerInformation findCustomersInformation(String projectCustomerId,String phone,String projectId) {
		...
		if(customer==null){
			customer = (ProjectCustomers) baseDao.loadObject("from ProjectCustomers where morePhoneNum='"+phone+"' and projectId='"+projectId+"'");
		}
		if(customer==null){
		...
		//性别
		pc.setSex(customer.getSex());
		pc.setDesc(customer.getDescription());
		pc.setMorePhone(customer.getMorePhoneNum());
		//客户名字
...
//		}
	...
	public ProjectCustomers updateCustomerAndTags(ProjectCustomers pc, Integer[] tagss) {
		...
		newpc.setMorePhoneNum(pc.getMorePhoneNum());
	...
	public ProjectCustomers findCustomerByPhone(String phone,String projectId) {
		...
		if(pc==null){
			pc = (ProjectCustomers)baseDao.loadObject("from ProjectCustomers where morePhoneNum='"+phone+"' and projectId='"+projectId+"'");
		}
		return pc;
	}
	...
	@Override
	public String findEvaluate(String customerId) {
		List<Comment> findByHql = baseDao.findByHql("from Comment where customerId = '"+customerId+"' order by commentTime DESC");
		if(findByHql.size()>0){
			return findByHql.get(0).getCustomerComment();
		}
		return null;
	}
