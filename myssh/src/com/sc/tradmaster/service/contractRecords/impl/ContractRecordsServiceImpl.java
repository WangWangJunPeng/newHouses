...
public class ContractRecordsServiceImpl implements ContractRecordsService {
			...
			if (grList.size()>0){
				for (int i = 0; i < grList.size(); i++) {
					if (grList.get(i).getApplyStatus() == 1){
						...
						HashMap grMap = JSON.parseObject(grList.get(i).getRules(), HashMap.class);

						Integer validDays = Integer.parseInt(grMap.get("validDays").toString());
						List<VisitRecords> vrList = baseDao.findByHql("from VisitRecords where phone = '"+grList.get(i).getCustomerPhone()+"' and arriveTime >= '"+grList.get(i).getApplyTime()+"' and projectId = '"+grList.get(i).getProjectId()+"' order by arriveTime");
						for (int j = 0; j < vrList.size(); j++) {
							if (j == 0){
								//到期时间
								String daoqiTimeStr = DateUtil.addDateMinut(sdf.format(applyTimeDate), validDays);
								Date daoqiTime = DateUtil.parse(daoqiTimeStr);
								Date x = sdf.parse(vrList.get(j).getArriveTime());
								long y = x.getTime() - daoqiTime.getTime();
								if (y <= 0){
									Date protectTime = DateUtil.rollMonth(applyTimeDate, Integer.parseInt(grMap.get("custormerProtectDays").toString()));
									long z = d.getTime() - protectTime.getTime();
									if (z <= 0){
										scdto.setIsChoose(1);
									}else {
										scdto.setIsChoose(0);
	 ...
	 */
	@Override
	public List findMidDeal(User user, String projectId, String startTime, String endTime) throws ParseException {
		// 设置时间格式
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Date date5 = new Date();
		List<ContractRecords> inList = new ArrayList<>();
		...
		if (crList.size()>0){
			for (ContractRecords contractRecords : crList) {
				...
				if (contractRecords.getGuideId() != null && !"".equals(contractRecords.getGuideId())){
					...
					GuideRecords gr = (GuideRecords) baseDao.loadObject("from GuideRecords where recordNo = '"+recordNo+"' ");
		...
		for (int i = 0; i < inList.size(); i++) {
			for (int j = 0; j < inList.size() -1-i; j++) {
				if (sdf.parse(inList.get(j).getApplyTime()).getTime() > sdf.parse(inList.get(j).getApplyTime()).getTime()) {
					ContractRecords contractRecords = inList.get(j);
					inList.set(j, inList.get(j+1));
					inList.set(j+1, contractRecords);
				}
	...
	public void addOrUpdateContract(User user, String desc, Integer credentialsType, Integer recordNo,
		...
		if (cr!= null && !"".equals(cr)){
			...
			//记录流水
			flowRecordsService.addOdersFR(user,1003,2003,cFr,cr);
			Project p = (Project) baseDao.loadById(Project.class, cr.getProjectId());
			...
			//更改房源状态__________________________
			
			House house33 = (House) baseDao.loadObject("from House where houseNum = '"+cr.getHouseNum()+"' ");
			if (house33.getHouseStatus() != 7 && house33.getHouseStatus() != 8){
				house33.setHouseStatus(6);
				baseDao.saveOrUpdate(house33);
			}
	...
	public void addNewContractRecord(User user, Integer orderProperty,Integer isAlreadyRead,Integer payStyle,
			Integer accountStyle,String benefitInfo,Integer houseNum,String buyPrice,String customerId,String rengourenIdCard,String realCustomerId,String dposit) throws IOException, ParseException {
		...
		for (Role role : roleSet) {
			...
			if (role.getRoleId() == 3) {
				...
				if (pc != null && !"".equals(pc)){
					...
					String grHQL = "from GuideRecords where customerPhone = '"+pc.getProjectCustomerPhone()+"' and projectId = '"+pc.getProjectId()+"' and applyStatus = 1 order by applyTime DESC";
					List<GuideRecords> grList = baseDao.findByHql(grHQL);
					if (grList.size()>0) {
						for (int i = 0; i < grList.size(); i++) {
							if (i == 0){
								String applyTime = grList.get(i).getApplyTime();
								Date applyTimeDate = sdf2.parse(applyTime);
								HashMap grMap = JSON.parseObject(grList.get(i).getRules(), HashMap.class);

								Integer validDays = Integer.parseInt(grMap.get("validDays").toString());
								//到期时间
								String daoqiTimeStr = DateUtil.addDateMinut(applyTime, validDays);
								Date daoqiTime = DateUtil.parse(daoqiTimeStr);
								...
								for (int j = 0; j < vrList.size(); j++) {
									if (j == 0){
										...
										if (y <= 0){
											...
											if (z <= 0){
												//添加备案号
												cr.setGuideId(grList.get(i).getRecordNo()+"");
												
												Mydynamic md2 = new Mydynamic();
												md2.setUserId(grList.get(i).getUserId());
//													md2.setUserPhone(grList.get(i).getPhone());
												md2.setCustomerName(grList.get(i).getCustomerName());
												md2.setCustomerPhone(grList.get(i).getCustomerPhone());
												md2.setIsRead(0);
												md2.setCreatTime(DateTime.toString1(new Date()));
												md2.setProjectId(grList.get(i).getProjectId());
												Project p = (Project) baseDao.loadObject("from Project where projectId = '"+grList.get(i).getProjectId()+"' ");
												md2.setProjectName(p.getProjectName());
//													md2.setHosueNum(cr.getHouseNum().toString());
												md2.setIsDelete("0");
												md2.setDynamicInfo("客户"+grList.get(i).getCustomerName()+"在案场:"+p.getProjectName()+"已提交认购申请!");
												md2.setDynamicStatus("认购申请");
												baseDao.save(md2);
											}
										}
									}
			...
			if (role.getRoleId() == 1 || role.getRoleId() == 2){
				House house2 = (House) baseDao.loadObject("from House where houseNum = '"+houseNum+"' ");
				String grHQL = "from GuideRecords where shopCustomerId = '"+customerId+"' and projectId = '"+house2.getProjectId()+"' and applyStatus = 1 order by applyTime DESC";
				List<GuideRecords> grList = baseDao.findByHql(grHQL);
				if (grList.size()>0) {
					for (int i = 0; i < grList.size(); i++) {
						if (i == 0){
		...
		baseDao.saveOrUpdate(cr);
		//添加流水
		flowRecordsService.addOdersFR(user,1001,2001,null,cr);
		Project p = (Project) baseDao.loadById(Project.class, cr.getProjectId());
		//new  动态
		Mydynamic md = new Mydynamic();
		md.setUserId(user.getUserId());
		...
		md.setHosueNum(cr.getHouseNum().toString());
		md.setIsDelete("0");
		md.setDynamicInfo("客户"+cr.getCustomerName()+"在"+p.getProjectName()+"已提交认购申请!");
		md.setDynamicStatus("认购申请");
		baseDao.save(md);
		
		//更改房源状态_____________
		House house2 = (House) baseDao.loadObject("from House where houseNum = '"+houseNum+"' ");
		if (house2.getHouseStatus() != 5 && house2.getHouseStatus() != 6 && house2.getHouseStatus() != 7 && house2.getHouseStatus() != 8 ){
			house2.setHouseStatus(4);
			
			baseDao.saveOrUpdate(house2);
		}
	 ...
	 */
	@Override
	public void updateContractRecordToRevoke(User u,Integer recordNo,String killTheOrderReason,String revokeOrderNotes) {
		ContractRecordsFR cFr = null;
		ContractRecords cr = (ContractRecords) baseDao.loadObject("from ContractRecords where recordNo = '"+recordNo+"'");
		cFr = new ContractRecordsFR().creatNewCrDTO(cr);
		if (cr != null && !"".equals(cr)){
		...
		baseDao.saveOrUpdate(cr);
		flowRecordsService.addOdersFR(u,1003,2007,cFr,cr);//记录流水信息
	}
		...
		if(user!=null){
			...
			for(Role r : role){
				if(r.getRoleId() == 1 || r.getRoleId() == 2){
					...
					if(pc!=null){
						...
						enterBuyMan = cr.getCustomerName();
						idCardNum = cr.getCustomerIDCard();
					}
			...
			if (cr.getOrderProperty() != null && !"".equals(cr.getOrderProperty())){
				if(cr.getOrderProperty().equals(1)){
					//真实认购人
					if (cr.getRealCustomerId() != null && !"".equals(cr.getRealCustomerId())){
	...
	@Override
	public void addPayByClient(User user, String enterBuyCusPhone, String enterBuyApplyTime,String proId,
			Integer enterBuyHouseNum) {
		// 根据recordNo查出订购记录表
		String orderHql = "from ContractRecords where 1=1 and userId = '"+user.getUserId()+"' and projectId = '"+proId+"' "
				+ " and houseNum = "+enterBuyHouseNum+" and customerPhone = '"+enterBuyCusPhone+"'";
		ContractRecords cr = (ContractRecords) baseDao.loadObject(orderHql);
		if (cr!= null && !"".equals(cr)){
			//String rename = SysContent.getFileRename(pic.getOriginalFilename());
			// 设置凭证url
			//String savePath = PicUploadToYun.upload(rename, pic,SmsContext.CR_PIC);
			cr.setCredentialsPhoto("AutoAdd");
			Date date = new Date();
			// 设置时间格式
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String voucherUploadTime = sdf.format(date);
