...
public class AdvertisementServiceImpl implements AdvertisementService {
	...
	public List getAdsByUserInfo(User user,String shi,String adPosition) {
		...
		if(user!=null && !"".equals(user)){
			...
			String hql = "from Advertisement where adPosition = '"+ adPosition + "' and startTime <= '"+nowTime+"'"
					+ " and endTime >= '" + nowTime + "' and state = 1";
	...
	public List<Advertisement> getAdsLocation(User user, String shi) {
		if (user != null) {
			...
			String[] cityStr = city.split("-");
			if (shi != null && !"".equals(shi)) {
				String hql = "from Advertisement where adCity like '%" + shi + "%' and adPosition = '" + "本地推荐" + "' and state = 1";
				List<Advertisement> adList = baseDao.findByHql(hql);
				return adList;
			} else {
				String hql = "from Advertisement where adCity like '%" + cityStr[1] + "%' and adPosition = '" + "本地推荐" + "' and state = 1";
				List<Advertisement> adList = baseDao.findByHql(hql);
				return adList;
			}
	...
	public List<Advertisement> getAdsForOthers(User user, String shi) {
		if (user != null) {
			...
			String[] cityStr = city.split("-");
			if (shi != null && !"".equals(shi)) {
				String hql = "from Advertisement where adCity like '%" + shi + "%' and adPosition = '" + "其他推荐" + "' and state = 1";
				List<Advertisement> adList = baseDao.findByHql(hql);
				return adList;
			} else {
				String hql = "from Advertisement where adCity like '%" + cityStr[1] + "%' and adPosition = '" + "其他推荐" + "' and state = 1";
				List<Advertisement> adList = baseDao.findByHql(hql);
				return adList;
			}
		}
