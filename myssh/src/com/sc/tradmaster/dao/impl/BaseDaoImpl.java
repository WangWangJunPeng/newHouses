...
public class BaseDaoImpl extends HibernateDaoSupport implements BaseDao{
	...
	public List queryBySql(String sql,Class clazz) {    
		Session session = super.getSessionFactory().getCurrentSession();
		List<Object[]> list = session.createSQLQuery(sql).addEntity(clazz).list();
        return list;
    }
	...
	public List findByPropertyToUpDown(String clazz, String[] propertyName,
			Object[] value, String[] orderName, String[] orderType, int start,
			int limit,Integer houseStatus) {
		String hql = "from " + clazz + " as model where 1=1";
		for (int i = 0; i < propertyName.length; i++) {
			if (null != value[i] && !value[i].equals("")) {
				hql += " and model." + propertyName[i] + " like '%" + value[i]
		...
		hql += " and (model.houseStatus = 0 or model.houseStatus = 1 or model.houseStatus = 4)";
		String order = "";
		for (int i = 0; i < orderName.length; i++) {
			if(i==0){
				order += " order by " ;
	...
	public List findByPropertyToBenefits(String clazz, String[] orderName, String[] orderType, int start,
			int limit,Integer houseStatus,String projectId) {
		String hql = "from " + clazz + " as model where 1=1 and model.projectId = '"+projectId+"' ";
		hql += " and (model.houseStatus = 0 or model.houseStatus = 1 or model.houseStatus = 4)";
		String order = "";
		for (int i = 0; i < orderName.length; i++) {
			if(i==0){
	...
	/** sum值统计 */
	public String findSum(String hql) {
		final String counthql = hql;
		Object sum = getHibernateTemplate().execute(new HibernateCallback(){
			public Object doInHibernate(Session session) throws HibernateException{
				Query query = session.createQuery(counthql);
				query.setMaxResults(1);
	...
	/** sum值统计
	public int findSumDouble(String hql) {
		final String counthql = hql;
		Double sum = getHibernateTemplate().execute(new HibernateCallback(){
			public Object doInHibernate(Session session) throws HibernateException{
				Query query = session.createQuery(counthql);
				query.setMaxResults(1);
				return query.uniqueResult();
	 ...
	 */
	@Override
	public Object queryObjBySql(String sql,Class clazz) {    
		Session session = super.getSessionFactory().getCurrentSession();
		List<Object[]> list = session.createSQLQuery(sql).addEntity(clazz).list();
		if(!list.isEmpty()){
			return list.get(0);
