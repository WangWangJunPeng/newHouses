...
public class AppTagManagerController extends BaseController{
	...
	public String toShowCustomersInfoByPhone(String index,String phone,Integer visitNo,RedirectAttributes ra){
		...
		if(user.getUserId().equals(pc.getOwn())){//属于自己的客户
			ra.addAttribute("projectId", projectId);
			ra.addAttribute("customerId",pc.getCustomerId());
			return "redirect:/toCustomerEdit";
		}else{
			ra.addAttribute("phone",pc.getCustomerPhone());
			return "redirect:/newCustomerEdit";
		}
	}
	...
	public Map<String,String> showManagerEvaluate(String customerId,String projectId){	
		...
		String evaluate = appTagManagerService.findEvaluate(customerId);
...
//			return "redirect:/findCustomersInformation";
	...
	public Map<String,Object> saveCustomerAndTagNewVersion(String index,ProjectCustomers pc,Integer visitNo,Integer[] tagss) throws Exception{
		...
		ProjectCustomers old = new ProjectCustomers();
		if(visitNo==null){
			//单纯保存
			old = appTagManagerService.updateCustomerAndTags(pc,array);
			res.put("data", pc.getProjectCustomerId());
		}else{
			//获取案场客户及更新
			old = projectCustomerService.findProjectCustomersById(pc.getProjectCustomerId(),pc);
			if(old!=null){
				if(!StringUtils.isEmpty(pc.getProjectCustomerId())){
					//保存已经存在的客户
					tagService.addTagRelation(array, pc.getProjectCustomerId());
					tagService.addProjectCustomerSex(pc.getSex(),pc.getProjectCustomerId());
					tagService.addProjectCustomerMorePhoneAndDesc(pc.getDescription(),pc.getMorePhoneNum(),pc.getProjectCustomerId());
					res.put("data", pc.getProjectCustomerId());
				}else{//不存在情况
					//保存不存在的客户
					tagService.addTagRelation(array, old.getProjectCustomerId());
					tagService.addProjectCustomerSex(pc.getSex(),old.getProjectCustomerId());
					tagService.addProjectCustomerMorePhoneAndDesc(pc.getDescription(),pc.getMorePhoneNum(),old.getProjectCustomerId());
					res.put("data", old.getProjectCustomerId());
				}
				//更新到访记录中的客户基本信息
				visitRecordsService.addOrUpdateVisitByVisitNo(visitNo,old);
			}else{
				res.put("msg", "客户信息异常");
			}
			//更新客户标签到到访记录
	...
	public Map<String,Object> saveCustomerAndTag(String index,ProjectCustomers pc,Integer visitNo,Integer[] tagss) throws Exception{
		...
		if(visitNo==null){
			...
			if(!StringUtils.isEmpty(pc.getProjectCustomerId())){
				...
				tagService.addProjectCustomerSex(pc.getSex(),pc1.getProjectCustomerId());
				tagService.addProjectCustomerMorePhoneAndDesc(pc.getDescription(),pc.getMorePhoneNum(),pc1.getProjectCustomerId());
