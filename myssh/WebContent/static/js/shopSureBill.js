...
function setShoperSureBillsInfo(data){
	...
	$.each(data,function(v, o) {
		//console.log(o.mediPayMoneytype);
		...
		// 门店到款操作按钮
		if (o.mediPayMoneytype== 1 || o.mediPayMoneytype== 0) {
			s += '<td><button class="layui-btn sure-payment" data-value='+o.recordNo+'>确认到款</button></td></tr>'
		} else if (o.mediPayMoneytype == 2) {
			s += '<td><button class="layui-btn cancel-payment" data-value='+o.recordNo+'>取消到款</button></td></tr>';
		} else {
			s += '<td></td></tr>';
		}
	});
...
function selectDate(){
	$('#selectSearch').html('<i class="layui-icon">&#xe63d;</i>');
	if($("#hidNum").val()==0){
		getShoperSureBillsList();
...
function toAnimationStart1(current){
	var interval = setInterval(increment,100);
	  

	  function increment(){
	    current++;
	    //$('#counter1').html(current+'%'); 
...
function toAnimationStart2(current){
	var interval = setInterval(increment,100);
	  

	  function increment(){
	    current++;
	   // $('#counter2').html(current+'%'); 
	    if(current == 100) { current = 0; }
	  }
...
function toAnimationStart3(current){
	var interval = setInterval(increment,100);
	  

	  function increment(){
	    current++;
	   // $('#counter3').html(current+'%'); 
	    if(current == 100) { current = 0; }
	  }
