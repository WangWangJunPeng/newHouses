...
function setBuyApplyList(data){
	...
	$.each(data,function(v,o){
		...
		s+= "<table cellpadding='5' id='applyBuyList'>";
			...
			if(o.orderProperty == 0){
				s+='<td>自购</td>';
			}else if(o.orderProperty == 1){
				s+='<td>代购</td>';
			}else{
				s+='<td>未填写</td>';
			}
			if(o.hadConRe=="yes"){
				s+='<td rowspan="3"><button style="background:#999" class="audit"  onclick="auditPop('+o.recordNo+')" disabled="disabled">同意</button>';
			}else{
				s+='<td rowspan="3"><button class="audit"  onclick="auditPop('+o.recordNo+')">同意</button>';
			}
