...
function setTodayNeedToDoList(data,num){
	if(num==0){
		...
		if(data.length>0){
			...
			$("#t_enterBuy").html("<br/><span style='width:10%;height:30px;display:block;margin:0 auto;'>暂无数据</span>");
		}
	}else if(num==3){
		...
		if(data.length>0){
			...
			$("#t_waitCash").html("<br/><span style='width:10%;height:30px;display:block;margin:0 auto;'>暂无数据</span>");
		}
	}else if(num==1){
		...
		if(data.length>0){
			...
			$("#t_getMoney").html("<br/><span style='width:10%;height:30px;display:block;margin:0 auto;'>暂无数据</span>");
		}
	}else if(num==2){
		var s = "<tr><th>地区</th><th>项目</th><th>申请人</th><th>打款时间时间</th><th>购买价格</th><th>定金</th><th>操作</th></tr>";
		$.each(data,function(v,o){
		...
		if(data.length>0){
			...
			$("#t_waitSign").html("<br/><span style='width:10%;height:30px;display:block;margin:0 auto;'>暂无数据</span>");
		}
	}
}
