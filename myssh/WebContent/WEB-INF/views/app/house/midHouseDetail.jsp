...
<link href="<%=request.getContextPath()%>/static/css/mui.css"
	rel="stylesheet" />
<link rel="stylesheet" type="text/css"
	href="<%=request.getContextPath()%>/static/css/app.css" />
<link rel="stylesheet" type="text/css"
	href="<%=request.getContextPath()%>/static/css/message.css">
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/static/css/houseDetail.css" />
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/static/css/suitable.css" />
<style type="text/css">
.mui-scroll-wrapper {
	overflow: auto;
}
...
<script src="<%=request.getContextPath()%>/static/js/flexible.js"
...
<script type="text/javascript">
	$.fn.ImgZoomIn = function() {
		...
		//alert($(this).attr('src'));
		imgstr = '<img id="ImgZoomInImage" src="'
				+ $(this).attr('src')
				+ '" onclick=$(\'#ImgZoomInImage\').hide();$(\'#ImgZoomInBG\').hide(); style="cursor:pointer; display:none; position:fixed; z-index:10001;" />';
		...
		$('#ImgZoomInImage').css(
				'left',
				$(window).scrollLeft()
						+ ($(window).width() - $('#ImgZoomInImage').width())
						/ 2);
		$('#ImgZoomInImage').css(
				'top',
				$(window).scrollTop()
						+ ($(window).height() - $('#ImgZoomInImage').height())
						/ 2);
		$('#ImgZoomInBG').show();
		$('#ImgZoomInImage').show();
	};
	...
	function renderHouseInfo(data) {
		...
		$
				.each(
						...
						function(v, o) {
							...
							s += '<section><a href="to_jingjirenOneHouse?houseNum='
									+ o.houseNum
									+ '"><h3 class="title">'
									+ o.buildingNo
									+ '栋'
									+ o.unit
									+ '单元'
									+ o.houseNo + '号(' + o.dealNum + ')</h3>';
							s += '<p><img src="static/images/icon1APP.png" alt=""><span>'
									+ ' ' + o.houseStyle + '</span>';
							s += '<span>  |  面积' + o.buildArea
									+ '平方米</span></p>';
							s += '<p><img src="static/images/icon2APP.png" alt=""><span></span><span>'
									+ ' ' + num + '元/平方米' + '</span>';
							s += '<span>  |  总价共</span><span>' + o.listPrice
									+ '</span><span>元</span></p></a>';
							if (o.isSupport == 1) {
								s += '<a class="successLink" href="to_goToContractRecord?houseNum='
										+ o.houseNum + '">发起认购</a>';
							} else {
		...
		if (data.length > 0) {
			...
			$("#selectHouseInfo")
					.html(
							"<tr><td style='width:10%;height:30px;margin:0 auto;'>暂无数据</td></tr>");
		}
	}
...
<body class="mui-ios mui-ios-9 mui-ios-9-1" style="padding-top: 0;">
	...
	<div class="mui-content">
		<input type="hidden" name="projectId" value="${dataInfo }"
			id="projectId">
		<div class="message">
			<section class="listStyle contain">
				<div class="leftbox">
					<img
						src="<%=request.getContextPath()%>/static/images/morentupian.png"
						alt="" class="exampleImg" id="adPhoto"
				...
				<div class="rightbox">
					...
					<a class="btn-see"
						href="to_addFavoriteToCollection?projectId=${dataInfo }">发起备案</a>

					<p class="list2">
						可售房源<span id="unitCount"></span><span>套</span>
					</p>
				</div>
				<div class="buttom-box">
					<a href="get_projectGuideRule?projectId=${dataInfo }"> <span
						class="guize">分销规则</span>
						<ul>
							<li id="isDaiKan"></li>
							<li id="isYiDi"></li>
							<li id="isFast"></li>
							<li>备案有效<span id="validDays"></span>小时
							</li>
		...
		<div id="slider2" class="mui-slider">
			<div id="sliderSegmentedControl"
				...
				<a class="mui-control-item" href="#item1mobile"> 项目详情 </a> <a
					class="mui-control-item" href="#item2mobile"> 房源列表 </a>
			</div>
			...
			<div class="mui-slider-group">
									...
									<div class="banner">
										<div id="slider" class="mui-slider">
											<div class="mui-slider-group mui-slider-loop"
												style="transform: translate3d(-9.2rem, 0rem, 0rem) translateZ(0rem); transition-duration: 0.2s">
				...
				<div id="item2mobile" class="mui-slider-item mui-control-content">
						...
						<div class="mui-scroll">
							<div class="page page2">
								<div class="selectNav">
									...
									<select class="mui-btn btn-block1" name="houseType"
										id="option-one">
					...
					<option value="item-6">一室一厅两卫</option> -->
									</select> <select class="mui-btn btn-block2" name="usefulArea"
										id="option-two">
					...
					<option value="">一室一厅两卫</option> -->
									</select> <select class="mui-btn btn-block" name="floor"
										id="option-three">
					...
					<option value="">三室两厅一卫</option>
					<option value="">一室一厅两卫</option> -->
									</select> <span class="mui-icon mui-icon-arrowdown pulldown1"></span> <span
										class="mui-icon mui-icon-arrowdown pulldown2"></span> <span
										class="mui-icon mui-icon-arrowdown pulldown3"></span>
								</div>
								<div class="messageList" id="selectHouseInfo"></div>
							</div>
						</div>
					</div>
