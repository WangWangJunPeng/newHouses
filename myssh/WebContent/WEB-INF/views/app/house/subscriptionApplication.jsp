...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="static/css/neworderStatus.css"/>
	...
	<script type="text/javascript">
	 mui.init();
		...
		function getAllMidCustomer(){
			window.location.href="to_goToMidMyCustomer?houseNum="+$('#houseNum').val();
		}
		...
		function getToSubmitContractRecord(){
			...
			$('.submitAudit').css('background',"#929292");
			$.ajax({
   				type : "post",
   				async : false,
		...
		function getReturn(){
			window.location.href="to_midOneProject?projectId="+$('#projectId').val();
		}
	</script>
</head>
<body class="mui-ios mui-ios-9 mui-ios-9-1">
	<header class="mui-bar mui-bar-nav mui-bar-transparent">
		<a class="mui-icon mui-icon-left-nav mui-pull-left" onclick="getReturn()"></a>
		<h1 class="mui-title">认购申请</h1>
	</header>
	<div class="mui-content">
	<form action="" method="post" id="myform" name="form">
	<input type="hidden" value="${sessionScope.newCrdto.projectId}" id="projectId" name="projectId">
			...
			<div class="mainMes">
				...
				<div class="line1">
					...
					<input type="hidden" value="${sessionScope.newCrdto.customerId}" id="customerId" name="customerId">
						<c:if test="${sessionScope.newCrdto.rName !=null && sessionScope.newCrdto.rName !=''}">
							<button type="button"  class="selectCustomer" onclick="getAllMidCustomer()" value="${sessionScope.newCrdto.customerId }" id="customerId" name="customerId">${sessionScope.newCrdto.rName }</button>
						</c:if>
						<c:if test="${sessionScope.newCrdto.rName ==null || sessionScope.newCrdto.rName ==''}">
							<button type="button" id="customerId" class="selectCustomer" onclick="getAllMidCustomer()" value="选择认购客户">选择认购客户</button>
						</c:if>
					</div>
				</div>
				...
				<div class="line1 tureCus">
					...
					<input type="hidden" value="${sessionScope.newCrdto.realCustomerId }" id="realCustomerId" name="realCustomerId">
						...
						<button type="button" class="writeCustomer" onclick="getChooseRealMan()" disabled="disabled">选择</button>
					</div>
				</div>
			...
			<form action="" class="contain">
				...
				<div class="midde">
					<div class="mui-input-row mui-checkbox mui-left">
  						<label style="margin-top:0;">我已阅读并已告知客户各条款内容</label>
		...
		$(document).ready(function(){
			...
			function beforewancheng(){
				...
				$("body").change(function(){
			  		if($("#customerId").val()!="选择认购客户"&&$("#rengourenIdCard").val()!=""&&$("#buyPrice").val()!==""&&$("#dposit").val()!==""&&$('#accountStyle').val()!=""&&$("#isAlreadyRead").is(':checked')==true&&$("#rengourenIdCard").val().length=="18"){
						$("#succ-Btn").removeClass("faileded").addClass("success").removeAttr("disabled");
			  		}else {
			  			$("#succ-Btn").removeClass("success").addClass("faileded").attr("disabled","disabled");
			  		}
