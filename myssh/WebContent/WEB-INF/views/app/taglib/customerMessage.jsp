...
<head>
	...
	$(function(){
		...
		$("#aditCustomerBtn").click(function(){
			...
			var own = $("#own").val();
			if(own==1){
				window.location.href="toCustomerEdit?customerId="+customerId+"&projectId="+projectId+"&index="+index;
			}else{
				//不是自己的客户
				$('#tishi').show();
			}
	...
	function status(){
		...
		var yidaofang = $("#yidao").data("value");
		if(yidaofang>0){
               ...
               success: function (data) {
            	   ...
            	   if(num==1){
            		   var t = '<div class="list">'+
							'<div class="leftbox"><span></span><div class="title">备注</div></div>'+
							'<div class="rightbox">'+
								'<p class="messList desc">'+
								'${customer.desc}'+
								'</p>'+
							'</div>'+
						'</div>';
            		   $(t).appendTo($("#item2mobile .contain"));
            	   }
	...
	function choicepage() {
		$("#indexNU").click(function(){
			...
			if(index=="5"){
				mui.openWindow({
				    url: "realCustomerList?index=5"
				  });
			}
			if(index=="6"){
				mui.openWindow({
				    url: "realCustomerList?index=6"
				  });
			}
			if(index=="7"){
				mui.openWindow({
				    url: "realCustomerList?index=7" 
				  });
			}
			if(index=="8"){
				mui.openWindow({
				    url: "realCustomerList?index=8"
				  });
			}
			if(index=="10"){
				mui.openWindow({
				    url: "to_saleVisitList"
				  });
			}
			if(index=="11"){
				mui.openWindow({
...
<body class="mui-ios mui-ios-9 mui-ios-9-1">
	<header class="mui-bar mui-bar-nav mui-bar-transparent">
		<a id="indexNU" class="mui-icon mui-icon-left-nav mui-pull-left"></a>
	...
	<input id="index" type="hidden" value="${index}">
		...
		<div class="people">
			...
			<div class="right">
				...
				<p class="top1"><span id="yidao" class="yuanyuan" data-value="${customer.comeNum}">已到访${customer.comeNum}次</span>
						...
						<div class="mui-scroll">
							<div class="contain">
								<div class="list">
									...
									<div class="rightbox">
										<p class="messList desc" >
										hsdkjfhsdkjfhsdjkhfsjkdhfsjkdfhkjsdf
										</p>
									</div>
								</div>
	...
	<div class="btn-success mui-bar mui-bar-tab">
			<div class="midde">
				<button type="button" id="firetBtn " ><img src="static/images/dianhua.png" style="width:0.5066rem;;height:height:0.3733rem;margin-right:0.2rem;vertical-align: middle;" alt="" /><span>电话</span></button>
				<button type="button" id="btn-succ" ><img src="static/images/xinxi.png" alt="" /><span>短信</span></button>
			</div>
	</div>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			/* mui('#scroll1').slider().setStopped(true)
			mui('#scroll2').slider().setStopped(true)
			mui('#scroll3').slider().setStopped(true) */
			...
			mui('.mui-slider').slider().setStopped(true)
			mui('.mui-slider').slider().setStopped(true) */
			/* $("#oneDiv").bind("click", function () {
				showOneDiv();
			});
			$("#twoDiv").bind("click", function () {
				showTwoDiv();
			});
			$("#threeDiv").bind("click", function () {
				managerEvl();
			}); */
