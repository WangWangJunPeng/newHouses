...
<head>
    ...
    <script type="text/javascript">
    			...
    			$.each(data,function(v, o) {
    				if(o.tagLibs.length>0){
    					...
    					if(flag==1){
    						...
    						$("#customerDemand").prepend(tagTypeDiv)
    					}
    				}
    	...
    	function checkMobile(phone){
    		console.log(phone);
    		var tl = /(^1[0-9]{10}$)/;
  			var p = ${customer.projectCustomerPhone};
    		if (phone == p) {
    			$('#tishi').show();		
				$('#tishi_p').html("该号码在被案场已被使用，请核实");
		        return false; 
    		}else if(tl.test(phone)){ 
		    	return true; 
		    }else if(phone == ""){
		    	return true; 
		    }else {
...
<body class="mui-ios mui-ios-9 mui-ios-9-1">
							...
							<div class="listes">
									...
									<div class="all1">
										<div class="contain">
											...
											<div class="list2">
												<label for="phone" class="left1">联系方式1</label>
												<input type="text" name="phone" id="phone" readonly="readonly" placeholder="输入联系方式" value="${customer.projectCustomerPhone}"/>
											</div>
											<div class="list2">
												<label for="phone" class="left1">联系方式2</label>
												<input type="text" name="phone2" id="phone2"  placeholder="输入联系方式" value="${customer.morePhoneNum}"/>
											</div>
											<div class="list2">
					...
					<div id="scroll2" class="mui-scroll-wrapper">
						<div id="customerDemand" class="mui-scroll">
							...
							<div class="listes">
									<div class="mes">
										<div class="tip leftbox">
											<span></span><div class="title">备注</div>
										</div>
									</div>
									<div class="all">
										<div class="contain">
											<textarea id="desc" placeholder="不超过200字..."  value="${customer.description}">${customer.description}</textarea>
										</div>
									</div>
								</div>
	...
	<div class="maskStu" id="tishi">
		<div class="tishi_content">
			<h1>提示</h1>
			<p id="tishi_p">该号码在被案场已被使用，请核实</p>
		...
		$(document).ready(function(){
			...
			$("#btn-next").click(function(){
				if ($("#phone2").val() == "") {
	    			...
	    			var check = checkMobile($("#phone2").val());
	        		if (check){
	        			$.ajax({
	        				url : 'matchCustomerPhone',
	        				type : 'post',
	        				dataType : 'json',
	        				data : {
	        					phone:$("#phone2").val()
