	      	...
	      	//删除选择弹框
       		...
       		function fillInForm(data){
       			if(data.hasOwnProperty("houseTypeId")){
					...
					$("#area").val(data.areaStr);
					$("#housTypeDesc").val(data.housTypeDesc);
					//var str = data.housType.split(" ");
					var type = data.housType;
					var str = [];
					if(type.length>=6){
						console.log(type);
						var fIndex = type.indexOf("房")+1;
						var tIndex = type.indexOf("厅")+1;
						var wIndex = type.indexOf("卫")+1;
						var fStr = type.substring(0,fIndex);
						var tStr = type.substring(fIndex,tIndex);
						var wStr = type.substring(tIndex,wIndex);
						str.push(fStr);
						str.push(tStr);
						str.push(wStr);
					}
